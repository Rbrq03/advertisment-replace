...
def make_dataset(split, dataname, istrain = True):   
    ...
    if dataname == 'coco':
        ...
        split_data_list = "list/adrecover/adrecover_split0.pth"
...
def load_fewshot_val1000(list_dir, name, split, shot, dataname = None, root = None):
    ...
    for record in dataset_dicts[:1000]:
        ...
        if dataname == 'adrecover':
            for i in range(1,6):
                support_image_path_list.append(f'data/adrecover/support_image/image{i}.jpg')
                support_label_path_list.append(f'data/adrecover/support_mask/mask{i}.png')
        else:
            for k in range(shot):
